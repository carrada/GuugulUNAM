...
class Z extends d {
  ...
  setColors(e) {
    if (Array.isArray(e) && e.length > 1) {
      const palette = e.map((col) => new l(col))
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      try {
        ...
        console.error("Ballpit no pudo iniciar WebGL:", error)
        spheresInstanceRef.current = null
      }
    }

    // StrictMode runs effect → cleanup → effect. Defer so the first
    // (discarded) pass never force-inits a context on this canvas.
